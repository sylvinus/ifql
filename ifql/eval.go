...
func (ev evaluator) eval(program *ast.Program, scope *Scope) error {
	for _, imp := range program.Imports {
		if err := ev.doImport(imp, scope); err != nil {
			return err
		}
	}
...
func (ev evaluator) doImport(imp *ast.ImportDeclaration, scope *Scope) error {
	p, err := ev.imp.Import(imp.Path.Value, ".")
	if err != nil {
		return errors.Wrapf(err, "failed to import %q", imp.Path.Value)
	}
	if !p.Complete() {
		s := scope.Nest()
		if err := ev.eval(p.Program(), s); err != nil {
			return err
		}
		p.SetScope(s)
	}
